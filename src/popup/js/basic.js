...
async function renderList() {
  ...
  if (Object.keys(items).length !== 0) {
    ...
    for (let i in items) {
      ...
      edit.src = "/resource/edit.png";
      edit.classList.add("item-icon");
...
function addEditSettingWindowButtonFunction() {
  ...
  function closeSettingWindow() {
    itemSettingWindow.style.top =
      document.querySelector("html").style.getPropertyValue("--page-height");
...
async function editItem(rule, option) {
  if (option === 0) {
    // add item
...
export {
  itemSettingWindow, newItem, currentTab,
  editItem, renderList, addEditSettingWindowButtonFunction
};